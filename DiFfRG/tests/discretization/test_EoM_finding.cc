...
TEST_CASE("Test 2D EoM finding on CG Constant model", "[discretization][EoM][2d]")
{
  ...
  // p_prm.initial_x0[0] = 1.0;
  // p_prm.initial_x1[0] = 8.13886;
  // p_prm.initial_x0[1] = -1.0;
  // p_prm.initial_x1[1] = 4.06928;
  p_prm.initial_x0[0] = 1;
  p_prm.initial_x1[0] = 1;
  p_prm.initial_x0[1] = 3.2335;
  p_prm.initial_x1[1] = 8.10487;
  ...
  DYNAMIC_SECTION("parameters were: x0[0]=" << p_prm.initial_x0[0])
  {
    INFO("parameters were: x0[0]=" << p_prm.initial_x0[0]);
    INFO("parameters were: x0[1]=" << p_prm.initial_x0[1]);
    INFO("parameters were: x1[0]=" << p_prm.initial_x1[0]);
    INFO("parameters were: x1[1]=" << p_prm.initial_x1[1]);
    INFO("parameters were: expected_EoM[0]=" << expected_EoM[0]);
    INFO("parameters were: expected_EoM[1]=" << expected_EoM[1]);
    REQUIRE_THAT(EoM[0], Catch::Matchers::WithinAbs(expected_EoM[0], 1e-3));
    REQUIRE_THAT(EoM[1], Catch::Matchers::WithinAbs(expected_EoM[1], 1e-3));
  }
}
...
TEST_CASE("TEST 2D EOM finding on CG Constant model using a more realistic curve", "[discretization][EoM][2d]")
{
      ...
      {{"physical", {}},
       ...
       {"discretization",
        {{"fe_order", GENERATE(1, 2, 3, 4, 5)},
         ...
         {"grid", {{"x_grid", "0:0.1:2"}, {"y_grid", "0:0.1:2"}, {"z_grid", "0:0.1:1"}, {"refine", 0}}},
         {"adaptivity",
          {{"start_adapt_at", 0.},
           {"adapt_dt", 1e-1},
           {"level", 0},
  ...
  // p_prm.a[0] = 8.04438;
  // p_prm.a[1] = 5.99045;
  // p_prm.x_prime[0] = 1.00819;
  // p_prm.x_prime[1] = 0.89942;
  ...
  INFO("parameters were: m[0]=" << p_prm.m[0]);
  INFO("parameters were: m[1]=" << p_prm.m[1]);
  INFO("parameters were: a[0]=" << p_prm.a[0]);
  INFO("parameters were: a[1]=" << p_prm.a[1]);
  INFO("parameters were: x_prime[0]=" << p_prm.x_prime[0]);
  INFO("parameters were: x_prime[1]=" << p_prm.x_prime[1]);
  INFO("parameters were: expected_EoM[0]=" << expected_EoM[0]);
  INFO("parameters were: expected_EoM[1]=" << expected_EoM[1]);
